...
export default async function SettingsLayout({
  ...
  const [ob, t, ts] = await Promise.all([
    getSetting("onboarding"),
    getTranslations("settings"),
    getTranslations("settings.steps"),
  ]);
  const extra = [
    { key: "notifications", path: "/settings/notifications", required: false },
  ] as const;
  return (
    <>
                ...
                return (
                  <li key={s.key}>
                    ...
                    >
                      ...
                      <span className="num text-xs text-muted-foreground w-5">
                        {i + 1}
                      </span>
