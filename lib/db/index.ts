...
function createClient() {
  ...
  // transaction-mode pooler. Opening a connection through the pooler is expensive, so
  // connections are kept for a while rather than recycled every minute; the query guard
  // below is what deals with a connection that has gone dead.
  // (max_pipeline is parsed by the driver but absent from its Options type.)
  const options = { prepare: false, max_pipeline: 1, max: 5, idle_timeout: 30, max_lifetime: 60 * 15, connect_timeout: 10 };
